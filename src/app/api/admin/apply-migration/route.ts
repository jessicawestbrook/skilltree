...
// Only use this in development - in production, use Supabase migrations
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      // If exec_sql doesn't exist, try direct execution (this won't work in most cases)
      console.error('Migration error:', error);
      return NextResponse.json(
        { 
          error: 'Failed to apply migration', 
          details: error.message,
          hint: 'You may need to apply this migration directly in the Supabase dashboard SQL editor'
        },
        { status: 500 }
      );
    }
