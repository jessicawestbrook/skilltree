...
const SpellingBeePage: React.FC = () => {
  ...
  const organizeStudyLists = (lists: StudyList[]) => {
    ...
    const gradeLists = lists.filter(list => list.name.includes('Grade') && list.name.includes('Spelling'))
    ...
    return { scrippsLists: sortedScrippsLists, gradeLists: sortedGradeLists }
  }

  // Load available study lists
  const loadStudyLists = useCallback(async () => {
    try {
      // Get public study lists for spelling bee (Scripps + Grade Level Spelling)
      const { data, error } = await supabase
        .from('study_lists')
        .select('*')
        .eq('is_public', true)
        .or('name.ilike.%Scripps%,name.ilike.%Spelling Words%')
        .order('name')
    ...
    <div className="max-w-4xl mx-auto p-1 sm:p-2 pt-2 sm:pt-2 space-y-1 sm:space-y-2">
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-lg sm:rounded-xl shadow-lg p-2 sm:p-3 pt-14 sm:pt-12 relative">
        ...
        <div className="text-center mb-1 sm:mb-2">
          <h1 className="text-lg sm:text-xl font-bold text-neutral-800 dark:text-neutral-200">Spelling Bee Practice</h1>
        </div>
        {/* Action Buttons */}
        {currentWord && (
          <>
              ...
              <div className="flex items-center gap-1">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {(() => {
                      const { scrippsLists, gradeLists } = organizeStudyLists(studyLists)
                      return (
                        <>
                          {scrippsLists.length > 0 && (
                            <optgroup label="Scripps National Spelling Bee">
