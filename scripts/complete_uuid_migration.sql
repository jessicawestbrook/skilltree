...
-- This version handles ALL dependent tables and views
...
-- Step 6: Recreate views with UUID types
-- First check what columns exist in learning_content and questions tables
DO $$
DECLARE
    has_lc_node_id BOOLEAN;
    has_lc_skill_id BOOLEAN;
    has_q_skill_id BOOLEAN;
    questions_join_column TEXT;
BEGIN
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'learning_content' AND column_name = 'node_id'
    ) INTO has_lc_node_id;
    
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'learning_content' AND column_name = 'skill_id'
    ) INTO has_lc_skill_id;
    
    -- Check for skill_id column in questions table
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'questions' AND column_name = 'skill_id'
    ) INTO has_q_skill_id;
    
    -- If skill_id doesn't exist, check for other possible column names
    IF NOT has_q_skill_id THEN
        -- Check for skill_tree_node_id
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'questions' AND column_name = 'skill_tree_node_id') THEN
    ...
    -- Build the view query dynamically based on what columns exist
    DECLARE
        view_query TEXT;
        lc_join TEXT := '';
        q_join TEXT := '';
    BEGIN
        -- Build learning_content join clause
        IF has_lc_node_id AND has_lc_skill_id THEN
            lc_join := 'LEFT JOIN learning_content lc ON lc.node_id = stn.id OR lc.skill_id = stn.id';
        ELSIF has_lc_skill_id THEN
            lc_join := 'LEFT JOIN learning_content lc ON lc.skill_id = stn.id';
        ELSIF has_lc_node_id THEN
            lc_join := 'LEFT JOIN learning_content lc ON lc.node_id = stn.id';
        END IF;
        
        -- Build questions join clause
        IF questions_join_column IS NOT NULL THEN
            q_join := format('LEFT JOIN questions q ON q.%I = stn.id', questions_join_column);
        END IF;
        
        -- Build the complete view query
        view_query := format('
        ...
        RAISE NOTICE 'View created successfully with joins: LC=%, Q=%', 
            CASE WHEN lc_join != '' THEN 'YES' ELSE 'NO' END,
            CASE WHEN q_join != '' THEN 'YES' ELSE 'NO' END;
    END;
END $$;
