// Import all processed spelling bee batches to database - ROBUST VERSION
// Handles multiple CSV formats and all edge cases
...
function mapCSVToDatabase(csvRow, format) {
  ...
  if (format === 'format1') {
    ...
    const aiDifficultyLevel = difficultyLevel; // Use same as difficulty level
    ...
    mappedData = {
      ...
      definition: csvRow.definition || null,
      example_sentence: csvRow.example_sentence || `Please spell the word _____.`,
      source_difficulty: 'One Bee', // Default since not provided in format2
      ...
      // Map the different score column names
...
async function importAllBatches() {
  console.log('🚀 Starting ROBUST import of all spelling bee batches...');
