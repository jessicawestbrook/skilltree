...
async function examineCreativeSkills() {
    try {
        console.log('Examining Creative Skills structure...\n');
        
        // Find Creative Skills node
        const { data: creativeSkillsNode, error: findError } = await supabase
        ...
        if (findError) {
            console.error('Error finding Creative Skills node:', findError);
            return;
        }
        
        console.log('Creative Skills root node:', {
            id: creativeSkillsNode.id,
            name: creativeSkillsNode.name,
            learning_area: creativeSkillsNode.learning_area,
            parent_id: creativeSkillsNode.parent_id
        });
        
        // Get all descendants recursively
        await getDescendants(creativeSkillsNode.id, '', 0);
...
async function getDescendants(parentId, parentName, depth) {
    try {
        ...
        if (children && children.length > 0) {
            ...
            console.log(`\n${indent}Children of ${parentName || 'Creative Skills'}:`);
