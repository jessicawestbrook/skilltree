-- Create Jessica's user account with username
...
DECLARE
  ...
  user_username TEXT := 'jessicawestbrook88';  -- Or whatever username you want
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.profiles WHERE username = user_username) THEN
    RAISE NOTICE '⚠️  Username % is already taken', user_username;
    -- Generate a unique username
    user_username := user_username || '_' || substr(gen_random_uuid()::text, 1, 4);
    RAISE NOTICE 'Using alternative username: %', user_username;
  END IF;
  ...
  ELSE
    ...
    INSERT INTO auth.users (
      ...
      jsonb_build_object('username', user_username),  -- Store username in metadata
      false,
  ...
  SET 
    ...
    username = COALESCE(profiles.username, EXCLUDED.username),  -- Keep existing username if present
    updated_at = NOW();
  
  RAISE NOTICE '✅ Profile created/updated with username: %', user_username;
...
-- Verify the user was created with username
SELECT 
