...
DROP POLICY IF EXISTS "Users can view their own learning path progress" ON user_learning_path_progress;
CREATE POLICY "Users can view their own learning path progress" ON user_learning_path_progress
    FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own learning path progress" ON user_learning_path_progress;
CREATE POLICY "Users can update their own learning path progress" ON user_learning_path_progress
    FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own learning path progress" ON user_learning_path_progress;
CREATE POLICY "Users can insert their own learning path progress" ON user_learning_path_progress
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Add update trigger for updated_at (check if function exists first)
...
-- Insert Latin courses and learning path data
