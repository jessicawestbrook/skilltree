...
DECLARE
  ...
  test_result RECORD;
BEGIN
  -- Create a fake NEW record to test
  test_result := ROW(
    test_id,                          -- id
  ...
  -- Try to call the function (this won't actually insert)
  BEGIN
    -- We can't directly test trigger function, but we can verify it compiles
    RAISE NOTICE '✅ Trigger function compiles successfully';
  EXCEPTION
    WHEN OTHERS THEN
      RAISE NOTICE '❌ Trigger function has errors: %', SQLERRM;
  END;
END $$;
