#!/usr/bin/env python3
"""
Generate Level 2 & 3 Node Descriptions with Checkpointing
=========================================================

This script generates descriptions for second and third level nodes in the skill tree.
Features comprehensive checkpointing, recovery, and progress tracking.

Level 2: Main Categories (74 nodes, ~70 need descriptions)  
Level 3: Subcategories (374 nodes, ~370 need descriptions)
Total: ~440 nodes to process
...
Usage:
    python generate_level2_level3_descriptions.py --batch-size 20
    python generate_level2_level3_descriptions.py --resume
    python generate_level2_level3_descriptions.py --dry-run --limit 10
"""
...
class Level23ProgressTracker:
    """Enhanced progress tracker for Level 2 & 3 description generation."""
    
    def __init__(self, checkpoint_file='level2_level3_progress.json'):
...
def get_level2_level3_nodes(generator: EnhancedContentGenerator, processed_ids: set) -> Tuple[List[SkillTreeNode], List[SkillTreeNode]]:
    """Get Level 2 and Level 3 nodes that need descriptions."""
    try:
...
def main():
    """Main function for Level 2 & 3 description generation."""
    parser = argparse.ArgumentParser(description='Generate descriptions for Level 2 & 3 nodes')
    ...
    print("Level 2 & 3 Description Generator")
    print("=" * 50)
    ...
    tracker = Level23ProgressTracker()
    ...
    try:
        ...
        # Get target nodes
        level2_nodes, level3_nodes = get_level2_level3_nodes(generator, tracker.processed_ids)
        ...
        if total_nodes == 0:
            print("[SUCCESS] No nodes need descriptions!")
            return
        ...
        if not args.dry_run and not args.resume:
            confirm = input("\n[QUESTION] Proceed with generation? (y/N): ")
            if confirm.lower() != 'y':
        ...
        if args.dry_run:
            ...
            print(f"\n[SAVED] Descriptions saved to database!")
